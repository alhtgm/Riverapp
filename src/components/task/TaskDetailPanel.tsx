...
export default function TaskDetailPanel({ task, subjects, onClose }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await updateTask(task.id, { title, subject_id: subjectId, start_date: startDate, due_date: dueDate, status, memo })
      onClose()
    } catch (e: unknown) {
        ...
        <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: 18, flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              ...
              <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} style={inputStyle} onFocus={handleInputFocus} onBlur={handleInputBlur} />
            </div>
            <div>
              <label style={labelStyle}>締切日</label>
              <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} style={inputStyle} onFocus={handleInputFocus} onBlur={handleInputBlur} />
            </div>
          </div>
